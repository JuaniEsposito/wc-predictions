...
def fetch_statsbomb_events(match_ids=None):
    ...
    """
    print("ADVERTENCIA: Esta operación es pesada y lenta.")
    print("   Solo usar para análisis táctico profundo de partidos específicos.")

    if not match_ids:
        raise ValueError("Debes especificar IDs de partidos (ej: python fetch_statsbomb.py 3788741)")
    ...
    if failed_matches and not events_data:
        raise DataFetchError(
            f"Todos los partidos fallaron: {failed_matches}"
        )

    if not events_data:
        raise DataFetchError("No se extrajeron eventos de ningún partido")
    ...
    if failed_matches:
        print(f"ADVERTENCIA: {len(failed_matches)} partidos fallaron: {[m[0] for m in failed_matches]}")
